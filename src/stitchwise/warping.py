...
def warp_two_images(base_image: np.ndarray, image_to_warp: np.ndarray, H_2_to_1: np.ndarray):
    ...
    warped_image = cv2.warpPerspective(image_to_warp, warp_matrix, (out_w, out_h), flags=cv2.INTER_LINEAR)

    src_mask = np.ones((h2, w2), dtype=np.uint8) * 255
    mask_warped = cv2.warpPerspective(src_mask, warp_matrix, (out_w, out_h), flags=cv2.INTER_NEAREST)
