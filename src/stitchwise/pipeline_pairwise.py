...
def _run_pairwise(cfg: StitchingConfig) -> dict:
    ...
    if len(good_matches) < 4:
        failure_reason = f"Not enough good matches: {len(good_matches)}"
    else:
        ...
        if H_2_to_1 is None:
            ...
            if len(inlier_matches) < cfg.min_inliers:
                failure_reason = f"Inlier count too low: {len(inlier_matches)} < {cfg.min_inliers}"

    inlier_count = len(inlier_matches) if inlier_matches else int(sum(inlier_mask)) if len(inlier_mask) else 0
    inlier_ratio = float(inlier_count / max(len(good_matches), 1))

    compute_warp_preview = bool(getattr(cfg, "compute_warp_preview", True))
    ...
    if H_2_to_1 is not None and compute_warp_preview:
        try:
            ...
            warping_preview = blend_images(base_canvas, warped_image, mask_base, mask_warped)
    ...
    success = (failure_reason == "") and (
        (warping_preview is not None) if compute_warp_preview else (H_2_to_1 is not None)
    )
...
def _save_pair_outputs(cfg: StitchingConfig, debug: dict) -> dict:
    pair_name = f"{Path(cfg.image1).stem}_{Path(cfg.image2).stem}"
    ...
    save_image(out_dir / "stitched.jpg", preview)
    ...
    stats.update({
        ...
        "stitched_result": str(out_dir / "stitched.jpg"),
    })
