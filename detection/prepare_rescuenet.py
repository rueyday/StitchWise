...
def inspect_dataset(dataset_path: Path):
    ...
    # Find splits
    splits = [d.name for d in dataset_path.iterdir() if d.is_dir()]
...
def find_pairs(dataset_path: Path):
    ...
    Handles common folder structures used in the Kaggle RescueNet upload.
    """
    pairs = {}
    ...
    for split in ["train", "val", "test"]:
        ...
        if not split_dir.exists():
            for alt in split_alts.get(split, []):
                if (dataset_path / alt).exists():
        ...
        if img_dir is None or lbl_dir is None:
            print(f"      WARNING: Could not find image/label dirs in {split_dir}")
            continue
